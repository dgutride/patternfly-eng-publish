...
class SiteRepo {
  ...
  inferRepo () {
    ...
    return promise
    .then(() => {
      let parsed = gitUrlParse(this.pushRepo);
      this.pushRepoName = parsed.name;
      this.pushRepoOwner = parsed.owner;
  ...
  push(deployment) {
    ...
    .then(clean => {
      if (clean) {
        console.log('Site already published to:');
        console.log(deployment.githubPagesUrl);
