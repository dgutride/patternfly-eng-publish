...
class TravisEnvironment {
  ...
  getDeployKey() {
    ...
    return exec(`openssl aes-256-cbc -K ${encryptedKey} -iv ${encryptedIv} -in deploy_key.enc -out deploy_key -d`)
    .then(() => exec('chmod 600 deploy_key; eval `ssh-agent -s`; ssh-add deploy_key'));
  }
}
