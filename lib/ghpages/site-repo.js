  ...
  clone(clonePath) {
    ...
    .then(() => {
      if (this.remoteBranchExists) {
        return Promise.resolve(this.remoteBranchExists);
      } else {
        return this.initializeRemoteBranch()
        .then(exists => exists ? true : Promise.reject('Error initializing Remote Branch'))
      }
    })
    .then(() => {
      console.log('Cloning site repo...');
      if (this.isLocalClone) {
        return exec(`git clone . ${clonePath}`)
          .then(() => exec(`git -C ${clonePath} remote set-url origin ${this.pushRepo}`))
          .then(() => exec(`git -C ${clonePath} fetch --depth 1 origin ${this.pushBranch}`));
          .then(() => exec(`git -C ${clonePath} checkout -t -B ${this.pushBranch} origin/${this.pushBranch}`))
      } else {
        return exec(`git clone --single-branch --depth 1 --branch ${this.pushBranch} -- ${this.pushRepo} ${clonePath}`);
      }
    })
  }
